...
A <code>Risk Estimator</code> function that 
...
## Model
The machine learning model is used to continuously monitor a patient's heart rate and alert the patient if their heart rate begins to show signs for possible Post-Exertional Malaise (PEM) episodes. We chose to use Autoencoder for our model, as the Autoencoder algorithm allows us to detect for anomallies. We will detect for anomallous heart rates in two categories, when the user is at rest and when the user is active. When the user is at rest, a heart rate that is 15 BPM above the user's resting heart rate is considered a heart rate that is at risk for a PEM episode. When the user is active, we use the Anaerboic Threshold as our heart rate threshold for those experiencing fatigue. A person experiencing fatigue is at risk for a PEM episode when they exceed the Anaerobic Threshold for more than 2 minutes. The Anaerobic Threshold for men is defined as being (220 - AGE)*0.6, while the Anaerobic Threshold for women is defined as being (220 - (AGE*0.88))*0.6. The model will note when the user's heart rate begins to be above this threshold, and after this threshold has been exceeded for over 2 minutes, it will alert the user that they are at risk for a PEM episode.
To train our model, our team had to gather heart rate data sets from healthy individuals, as there is limited heart rate data on long COVID patients. We labeled the heart rate data using our threshold discussed above, giving any heart rate that would be considered a PEM risk a 1 and others a 0. We then scanned the data in 2 minute increments for any heart rates that would trigger a PEM episode. If there was a heart rate in the 2 minute window that was labeled with a PEM risk, that entire 2 minute window would receive a PEM label of 1 and would alert the user that they are at risk of experiencing a PEM episode.
